    <div>
        ...
        <div x-data="{ sidebarOpen: false }" class="flex h-screen bg-gray-200">
            ...
            <div :class="sidebarOpen ? 'translate-x-0 ease-out' : '-translate-x-full ease-in'" class="fixed inset-y-0 left-0 z-30 w-64 overflow-y-auto transition duration-300 transform bg-gray-900 lg:translate-x-0 lg:static lg:inset-0 -translate-x-full ease-in">
                ...
                <nav class="mt-10">
                    <a class="flex items-center px-6 py-2 mt-4 {{ Request::routeIs('home') && request('username') === $customer->username ? 'text-gray-100 bg-gray-700 bg-opacity-25' : 'text-gray-500 hover:bg-gray-700 hover:bg-opacity-25 hover:text-gray-100' }}" href="{{ route('home', ['username' => $customer->username]) }}">
                    ...
                    <a class="flex items-center px-6 py-2 mt-4 {{ Request::routeIs('my-content') && request('username') === $customer->username ? 'text-gray-100 bg-gray-700 bg-opacity-25' : 'text-gray-500 hover:bg-gray-700 hover:bg-opacity-25 hover:text-gray-100' }}" href="{{ route('my-content', ['username' => $customer->username]) }}">
                        <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <a class="flex items-center px-6 py-2 mt-4 {{ Request::routeIs('profile.show') && request('username') === $customer->username ? 'text-gray-100 bg-gray-700 bg-opacity-25' : 'text-gray-500 hover:bg-gray-700 hover:bg-opacity-25 hover:text-gray-100' }}" href="{{ route('profile.show', ['username' => $customer->username]) }}">
                        <svg class="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="flex flex-col flex-1 overflow-hidden">
                <header class="flex items-center justify-between px-6 py-4 bg-white drop-shadow-lg shadow-black border-indigo-600">
                    <div class="flex items-center">
                        ...
                        <div x-data="{ dropdownOpen: false }" class="relative">
                            <button @click="dropdownOpen = ! dropdownOpen" class="relative block w-8 h-8 overflow-hidden rounded-full shadow focus:outline-none">
                                ...
                                <img class="object-cover w-full h-full" 
                                    src="{{ Storage::url($customer->image_path) }}" 
                            ...
                            <div x-show="dropdownOpen" class="absolute right-0 z-10 w-48 mt-2 overflow-hidden bg-white rounded-md shadow-xl" style="display: none;">
                                <a href="{{ route('profile.show', $customer->username) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white">Profile</a>
                                <a href="{{ route('my-content', $customer->username) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-indigo-600 hover:text-white">Products</a>
